...
printProgressBar(0, max_continuation_iters, prefix = 'Progress:', suffix = 'Complete', length = 50)
for dir_factor in [+1]:
    ...
    while continuation_iter < max_continuation_iters and dx0_offset >= dx0_offset_tol:
        ...
        if target_achieved == True:
            ...
            eigvals = np.linalg.eigvals(STMs[-1][[0,1,3,4],:][:,[0,1,3,4]])
...
for traj in overall_trajvec_eigchange:
    ax.plot(traj[:,0], traj[:,1], 'k', label="Eigenvalue Change")
