...
north_eigvals,_ = np.linalg.eig(STMs[-1][[0,1,3,4],:][:,[0,1,3,4]])
print("north_eigvals",north_eigvals)

south_is = [*achieved_initial_state]
south_is[2] *= -1
south_is[5] *= -1
south_halo_traj,_,_ = rungekutta4(CR3BP_nondim, south_is, np.linspace(0,2*TOF*revs,500*revs), args=(mu,))
ax.plot(south_halo_traj[:, 0], south_halo_traj[:, 1], south_halo_traj[:, 2],label="Halo 4 Achieved")

_, STMs = STM_coupled_integration_vec(south_is,np.linspace(0,2*TOF,100),ode=CR3BP_CoupledSTMFunc_Simple,ode_args=(mu,))
south_eigvals,_ = np.linalg.eig(STMs[-1][[0,1,3,4],:][:,[0,1,3,4]])
print("south_eigvals",south_eigvals)
...
# XY
axs[0].plot(1 - mu, 0, 'go', label="$m_2$")
axs[0].plot(achieved_halo_traj[:, 0], achieved_halo_traj[:, 1],'-b',label="North Halo")
axs[0].quiver(achieved_halo_traj[0, 0],achieved_halo_traj[0, 1],achieved_halo_traj[1, 0]-achieved_halo_traj[0, 0],achieved_halo_traj[1, 1]-achieved_halo_traj[0, 1],color='b',label="North")
axs[0].plot(south_halo_traj[:, 0], south_halo_traj[:, 1],'-r',label="South Halo")
axs[0].quiver(south_halo_traj[0, 0],south_halo_traj[0, 1],south_halo_traj[1, 0]-south_halo_traj[0, 0],south_halo_traj[1, 1]-south_halo_traj[0, 1],color='r',label="South")
axs[0].set_aspect('equal', 'box')
# XZ
axs[1].plot(1 - mu, 0, 'go', label="$m_2$")
axs[1].plot(achieved_halo_traj[:, 0], achieved_halo_traj[:, 2],'-b',label="North Halo")
axs[1].quiver(achieved_halo_traj[0, 0],achieved_halo_traj[0, 2],achieved_halo_traj[1, 0]-achieved_halo_traj[0, 0],achieved_halo_traj[1, 2]-achieved_halo_traj[0, 2],color='b',label="North")
axs[1].plot(south_halo_traj[:, 0], south_halo_traj[:, 2],'-r',label="South Halo")
axs[1].quiver(south_halo_traj[0, 0],south_halo_traj[0, 2],south_halo_traj[1, 0]-south_halo_traj[0, 0],south_halo_traj[1, 2]-south_halo_traj[0, 2],color='r',label="North")
axs[1].set_aspect('equal', 'box')
# YZ
axs[2].plot(0, 0, 'go', label="$m_2$")
axs[2].plot(achieved_halo_traj[:, 1], achieved_halo_traj[:, 2],'-b',label="North Halo")
axs[2].quiver(achieved_halo_traj[0, 1],achieved_halo_traj[0, 2],achieved_halo_traj[1, 1]-achieved_halo_traj[0, 1],achieved_halo_traj[1, 2]-achieved_halo_traj[0, 2],color='b',label="North")
axs[2].plot(south_halo_traj[:, 1], south_halo_traj[:, 2],'-r',label="South Halo")
axs[2].quiver(south_halo_traj[0, 1],south_halo_traj[0, 2],south_halo_traj[1, 1]-south_halo_traj[0, 1],south_halo_traj[1, 2]-south_halo_traj[0, 2],color='r',label="North")
axs[2].set_aspect('equal', 'box')

axs[0].set(title="Earth Moon Halo Orbits - XY",
       xlabel="X",ylabel="Y")
axs[1].set(title="Earth Moon Halo Orbits - XZ",
       xlabel="X",ylabel="Z")
axs[2].set(title="Earth Moon Halo Orbits - YZ",
       xlabel="Y",ylabel="Z")
axs[0].legend()
axs[1].legend()
axs[2].legend()
