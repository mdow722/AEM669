...
def rungekutta4(f, y0, t, args=(), event_conditions=None, stop_at_events=False, event_amount_to_stop=1):
    n = len(t)
    y = np.zeros((n, len(y0)))
    tvec = np.zeros(n)
    y[0] = y0
            ...
            for j in range(len(y0)):
                if event_conditions[j] == True:
                    if y[i+1,j] * y[i,j] < 0:
                        ...
                        if stop_at_events == True and len(event_states) >= event_amount_to_stop:
                            y = y[0:i+2]
                            tvec = tvec[0:i+2]
