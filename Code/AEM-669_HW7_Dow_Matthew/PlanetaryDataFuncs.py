...
def Get3BodyCharacteristics(primary_name, secondary_name):
    ...
    Tstar = np.sqrt(((Lstar*1000)**3)/(G_dim * Mstar))
    mu = secondary_mass/Mstar


    return Lstar,Mstar,Tstar,mu

def Manual3BodyCharacteristics(primary_mu,secondary_mu,assumed_distance):
    Lstar = assumed_distance
    Mstar = (primary_mu + secondary_mu)/G_dim*1e9
    Tstar = np.sqrt(((Lstar*1000)**3)/(G_dim * Mstar))
    mu = secondary_mu/(primary_mu+secondary_mu)
    return Lstar,Mstar,Tstar,mu
